...
```
{
  ...
  hdValue: '',  // 非必填。有hidden的地方可设置此值。当自己的hidden或父类的hidden为true时，取此值；此值取出后（如properties的hidden设置为true），子节点不再遍列取值
  properties: { // 根节点：表单字段
    name: {
      value: '', // 必填：数据的值
      hidden: false, //是否隐藏此级，默认为显示
      ...
      group: "g", //字符串（如"g"可以是任意字符串），设置此值说明相邻的"同组"节点会排成一行，不换行
      unit: "%",  //字符串，对组件进行补充
    ...
    more2: {
      title: ...
      //widget: "array",  //简写：值有"array" or "array-table";有此值说明是一个动态数组，可添加、删除、排序
      widget: {
        name: "array",
        hasSort: true,  //是否有排序按钮，默认为true
        hasDelete: true,  //是否有删除按钮，默认为true
        hasAdd: true, //是否有添加按钮，默认为true
        maxlength: 10,  //不写或小于等于0代表不限制
